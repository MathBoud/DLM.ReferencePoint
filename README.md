...
This tool is intended as a guide to help stock assessment biologists in finding a set of methods that could apply to the available data like commercial landings, fishery length composition and biomass survey. A Canadian Technical Report of Fisheries and Aquatic Sciences is available at http://waves-vagues.dfo-mpo.gc.ca/Library/41033747.pdf and is provinding a user guide for the decision tool for the selection of methods to obtain indicators and reference points for data-limited stocks.  

## Decision Tool
Decision tree process to select a set of methods that could be use according to data availability. The first step is to classify a stock in one of the data-limited stock (DLS) categories depending on the availability of commercial landings, biomass index and catch-at-length time series. After that, different methods can be selected according to the specie life history traits and stock dynamic available, depending of which category the stock has been classified (DLS-A, DLS-B, DLS-C or DLS-D). 
...
### Length-based spawning potential ratio for data-limited stock assessment (LB-SPR)
