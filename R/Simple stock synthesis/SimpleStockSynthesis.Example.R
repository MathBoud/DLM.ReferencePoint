...
ggplot(data=BBmsy, aes(x=Year, y=SSS.result.B_BMSY)) + 
  ...
  geom_hline(yintercept=0.8, linetype="dashed", color = "yellow2", size=1.1) +
  geom_hline(yintercept=1, linetype="dashed", color = "green2", size=1.1) +
  annotate(geom="text", x=min(BBmsy$Year)+1, y=0.1, label="Décimé", color="red", size=5) +
  annotate(geom="text", x=min(BBmsy$Year)+1, y=0.35, label="Sévèrement \nsurpêché", color="orange", size=5) +
  annotate(geom="text", x=min(BBmsy$Year)+1, y=0.65, label="Surpêché", color="yellow2", size=5) +
  annotate(geom="text", x=min(BBmsy$Year)+4, y=0.87, label="Légèrement surpêché", color="greenyellow", size=5) +
  annotate(geom="text", x=min(BBmsy$Year)+1, y=1.1, label="En santé", color="green2", size=5) +
  xlab("Année") + 
  ylab("B/Bmsy") +
  theme_classic()
